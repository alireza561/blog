...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                "دیدگاه جدید",
                "دیدگاه جدیدی برای مقاله ی <<{}>> که شما نویسنده ی آن هستید ارسال شده : \n{}{}".
                format(article, current_site, reverse('blog:article', kwargs={'slug': article.slug})),
                to=[author_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                "دیدگاه جدید",
                "پاسخی برای دیدگاه شما در مقاله ی <<{}>> ثبت شده . برای مشاهده بر روی لینک زیر کلیک کنید : \n{}{}".
                format(article, current_site, reverse('blog:article', kwargs={'slug': article.slug})),
                to=[parent_email]
            )
            email.send()
